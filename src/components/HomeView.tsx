    ...
    <div className="flex flex-col gap-16">
      ...
      >
        ...
        <div className="flex flex-col justify-center sm:pt-2">
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4 text-[#E8E0D4]">
            Hi, I&apos;m <span className="text-[#D4A870]">Sendi Awaludin</span>.
          </h1>
          <p className="text-lg text-[#907860] max-w-2xl leading-relaxed">
            Welcome to my digital garden. I'm a software engineer and designer crafting aesthetic web experiences. Here, I write about technology, design, and trying to make sense of the web. 
          </p>
        </div>
      </motion.section>
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-bold tracking-tight text-[#E8E0D4]">Latest Writing</h2>
          <Link href="/blog" className="text-sm font-medium text-[#C0B090] hover:text-[#D4A870] transition-colors">
            View all →
          </Link>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {posts.slice(0, 4).map((post, i) => (
            <Link key={post.slug} href={`/blog/${post.slug}`}>
              ...
              >
                <div>
                  <h3 className="font-semibold text-lg mb-2 text-[#E8E0D4] group-hover:text-[#D4A870] transition-colors">{post.title}</h3>
                  <p className="text-[#907860] text-sm line-clamp-3 leading-relaxed mb-4">{post.excerpt}</p>
                </div>
                <time className="text-xs text-[#6A5A48] font-mono">{post.date}</time>
              </motion.div>
            </Link>
          ))}
        </div>
