...
export default async function BlogIndex() {
  ...
  return (
    <div className="py-12">
      <h1 className="text-4xl font-extrabold tracking-tight mb-8 text-[#E8E0D4]">
        Semua Artikel
      </h1>
      ...
      <div className="flex flex-col gap-8">
        {posts.map((post) => (
          <article key={post.slug} className="group flex flex-col gap-2">
            <time className="text-sm font-mono text-[#6A5A48]">
              {post.date}
            </time>
            ...
            <Link
              href={`/blog/${post.slug}`}
              className="text-sm font-medium text-[#D4A870] mt-2 hover:underline"
            >
              Baca selengkapnya →
            </Link>
          </article>
